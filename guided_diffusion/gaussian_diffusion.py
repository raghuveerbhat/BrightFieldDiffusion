...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, input_bf, t, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, input_bf, self._scale_timesteps(t), **model_kwargs)
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                if i % 100 == 0:
                    ...
                    viz.image(visualize(img.cpu()[0, 0,...]), opts=dict(caption="sample"+ str(i) ))
                    viz.image(visualize(out["pred_xstart"].cpu()[0, 0,...]), opts=dict(caption="Prediction X0"))
    ...
    def _vb_terms_bpd(
        self, model, x_start, x_t, input_bf, t, clip_denoised=True, model_kwargs=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t, input_bf, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
        kl = normal_kl(
    ...
    def training_losses(self, model, x_start, input_bf, advesarial, GAN_loss, optimizer_D, t, model_kwargs=None, noise=None):
        ...
        terms = {}
        # viz.image(visualize_1_1(input_bf.cpu()[0, 0,...]), opts=dict(caption="input_bf"))
        # viz.image(visualize_1_1(x_start.cpu()[0, 0,...]), opts=dict(caption="x_start"))
    ...
    def calc_bpd_loop(self, model, x_start, input_bf, clip_denoised=True, model_kwargs=None):
        """
